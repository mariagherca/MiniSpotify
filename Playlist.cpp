...
Playlist::Playlist(const Playlist& other)
    : name(other.name), creator(other.creator), description(other.description) {
...
void Playlist::display(bool detailed) const {
    ...
    std::cout << " " << songs.size() << " melodii | "
              << "Durata: " << totalSec/60 << "m " << totalSec%60 << "s\n"
              << "Genuri: ";
...
const std::vector<std::unique_ptr<Song>>& Playlist::getSongs() const {
    return songs;
}
...
size_t Playlist::getSongCount() const {
    return songs.size();
}
...
std::unique_ptr<Playlist> Playlist::createFromFile(const std::string& name, const std::string& filename) {
    ...
    while (file >> type >> std::ws && std::getline(file, title, ',') &&
           std::getline(file, artist, ',') && file >> duration >> std::ws) {
